...
def get_from_utc_time(epoch_time):
    date = datetime.datetime.fromtimestamp(epoch_time, pytz.timezone("Canada/Eastern"))
...
def getHtmlTable():
    ...
    sorted_response_json = sorted(
        filtered_response_json, key=lambda x: x['start'])
    ...
    table = pretty_table = make_tables_pretty(table)
    return pretty_table
